...
function tftearing!(du, u, p, t)
    ...
    φp3 = (1 / 0.5 * M) * ((-φ2 + K2 * φ) * γ - 0.5 * M * (φm3 - (Kp2 - ky^2) * φm1 - (Kp2 - ky^2) * φp1) - (-ddfoverf(t) * ψ - K2 * ψ + S * ((K2 / S) * ψ + 0.5 * M * (ψp1 + ψm1) + γ * ψ - k * f(t) * φ)) * k * f(t))
...
function bc!(res, u, p, t)
    res[1] = u(-L)[9]     # ψm(L)=0,    Dirichlet on left boundary
    res[2] = u(-L)[7]     # ψm-1(L)=0,  Dirichlet on left boundary
    res[3] = u(-L)[11]    # ψm+1(L)=0,  Dirichlet on left boundary
    res[4] = u(-L)[3]     # ϕm(L)=0,    Dirichlet on left boundary
    res[5] = u(-L)[1]     # ϕm-1(L)=0,  Dirichlet on left boundary
    res[6] = u(-L)[5]     # ϕm+1(L)=0,  Dirichlet on left boundary
    res[7] = u(L)[9]     # ψm(L)=0,    Dirichlet on left boundary
    res[8] = u(L)[7]     # ψm-1(L)=0,  Dirichlet on left boundary
    res[9] = u(L)[11]    # ψm+1(L)=0,  Dirichlet on left boundary
    res[10] = u(L)[3]    # ϕm(L)=0,    Dirichlet on left boundary
    res[11] = u(L)[1]    # ϕm-1(L)=0,  Dirichlet on left boundary
    res[12] = u(L)[5]    # ϕm+1(L)=0,  Dirichlet on left boundary
