...
tf_dae_prob = TwoPointBVProblem(fun, initial_guess, tspan, [γ_guess], fit_parameters=true)

# traced_sys = modelingtoolkitize(tf_dae_prob)
# tf_sys = mtkcompile(dae_index_lowering(traced_sys))

#grrr! modelingtoolkitize only supports ODE problems!